...
music_studio_setup.py 로 설치된 모델(MusicGen / ACE-Step)을 자동 감지해서
같은 인터페이스로 BGM 생성. 사용자는 모델 차이 신경 쓸 필요 X.

config:
  PROMPT : 음악 묘사 (영어 권장)
  DURATION_SEC : 길이 (초)
  GENRE : 장르 파트 (lo-fi, ambient, cinematic, edm 등)
  OUTPUT_DIR : 출력 위치 (디폴트: ~/connect-ai-music/output/)
"""
...
def _log(msg, kind="info"):
    prefix = {"info": "ℹ️", "ok": "✅", "warn": "⚠️ ", "err": "❌"}.get(kind, "")
...
def _generate_musicgen(setup, prompt, duration_sec, output_path):
    """MusicGen 사용(transformers 기반). 가볍다."""
    venv_python = setup.get("VENV_PYTHON")
    hf_id = setup.get("HF_ID", "facebook/musicgen-small")

    # MusicGen은 초당 50 토큰/초 (sample rate 32000Hz, 50hz token rate)
    # duration 을 max_new_tokens 환산
    max_tokens = max(64, int(duration_sec * 50))

    # v2.89.76 : outer f-string의 prompt!r 치환에서 quote 충돌하던 문제 수정.
    # 변수에 먼저 넣은 뒤 inner f-string에서 {{변수}} 형태로 참조 (literal { 이스케이프).
...
print('ℹ️ 디바이스: ' + str(device), file=sys.stderr, flush=True)
...
"""
    ...
    if not os.path.exists(wav_path):
        return False, "wav 파일 생성 실패"

    # wav 를 mp3 변환(ffmpeg 있을 때)
...
def _generate_acestep(setup, prompt, duration_sec, output_path):
    """ACE-Step 의 repo내 infer 스크립트 호출. 무겁다."""
    venv_python = setup.get("VENV_PYTHON")
    repo_dir = setup.get("ACE_STEP_DIR")

    # ACE-Step entry point 자동 검색
    ...
    if not os.path.exists(output_path):
        return False, "출력 파일 없음. ACE-Step 명령 형식에 오류가 있을 수 있음"
    return True, output_path
...
def main():
    ...
    if not setup.get("INSTALLED_AT"):
        print("❌ 음악 모델 미설치")
        print("  먼저 같은 폴더의 'music_studio_setup.py' 실행해주세요 (더블클릭).")
        print("  기본은 MusicGen Small (300MB) 로 가볍습니다.")
        sys.exit(1)

    venv_python = setup.get("VENV_PYTHON")
    if not (venv_python and os.path.exists(venv_python)):
        print("❌ 설치 정보 이상. music_studio_setup.py 다시 실행해주세요.")
        sys.exit(1)
